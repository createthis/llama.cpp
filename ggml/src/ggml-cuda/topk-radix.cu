...
static __global__ void k_select_topk_bins(const float * __restrict__ scores,
    ...
    // Staged refinement: second and third bytes to reduce eq set without large shared buffers
    int needed2 = k - sel_count;
    if (needed2 > 0) {
        __shared__ unsigned int h2[256];
        for (int i = threadIdx.x; i < 256; i += blockDim.x) h2[i] = 0;
        __syncthreads();
        // build histogram of second byte among top-byte equals
        for (int i = threadIdx.x; i < N; i += blockDim.x) {
            uint32_t key = float_to_key_desc(col[i]);
            int b0 = (key >> 24) & 0xFF;
            if (b0 == thr0) {
        ...
        __syncthreads();
        // find thr1 for second byte
        int thr1 = 0; {
            unsigned int sum = 0;
            for (int b = 255; b >= 0; --b) {
                unsigned int sgt = sum;
                unsigned int eq  = h2[b];
                if (sgt < (unsigned int)needed2 && sgt + eq >= (unsigned int)needed2) { thr1 = b; break; }
                sum += eq;
            }
        }
        // select those strictly greater on second byte
        for (int i = threadIdx.x; i < N; i += blockDim.x) {
            uint32_t key = float_to_key_desc(col[i]);
            int b0 = (key >> 24) & 0xFF;
            if (b0 != thr0) continue;
    ...
    int needed3 = k - sel_count;
    if (needed3 > 0) {
        __shared__ unsigned int h3[256];
        for (int i = threadIdx.x; i < 256; i += blockDim.x) h3[i] = 0;
        __syncthreads();
        // find thr1 again (in case branch above not executed on some warps)
        // compute second-byte threshold locally (cheap) to use in third-byte pass
        int thr1_local = 0; {
            unsigned int sum = 0;
            for (int b = 255; b >= 0; --b) {
                unsigned int sgt = sum;
                unsigned int eq  = 0; // cannot access h2 here portably across scopes; recompute h2 per thread
                // recompute h2[b] in chunks for this column
            }
        }
        // recompute thr1 by scanning N and counting second-byte histogram again (safe but extra pass)
        {
            unsigned int h2_local[256];
            for (int b = 0; b < 256; ++b) h2_local[b] = 0u;
            for (int i = threadIdx.x; i < N; i += blockDim.x) {
                uint32_t key = float_to_key_desc(col[i]);
                int b0 = (key >> 24) & 0xFF;
                if (b0 == thr0) {
                    int b1 = (key >> 16) & 0xFF;
                    atomicAdd(&h3[b1], 1u); // reuse h3 as scratch to merge across threads
                }
            }
            __syncthreads();
            // now h3 holds h2 histogram; compute thr1
            if (threadIdx.x == 0) {
                unsigned int sum = 0; int thr1_tmp = 0; unsigned int need = (unsigned int)(k - sel_count);
                for (int b = 255; b >= 0; --b) {
                    unsigned int sgt = sum;
                    unsigned int eq  = h3[b];
                    if (sgt < need && sgt + eq >= need) { thr1_tmp = b; break; }
                    sum += eq;
                }
                h3[0] = (unsigned int) thr1_tmp; // stash thr1 in h3[0]
            }
            __syncthreads();
            int thr1 = (int) h3[0];
            // clear h3 for third-byte histogram
            for (int i = threadIdx.x; i < 256; i += blockDim.x) h3[i] = 0u;
            __syncthreads();
            // build third-byte histogram conditioned on b0==thr0 and b1==thr1
            for (int i = threadIdx.x; i < N; i += blockDim.x) {
                uint32_t key = float_to_key_desc(col[i]);
                int b0 = (key >> 24) & 0xFF;
                if (b0 != thr0) continue;
            ...
            __syncthreads();
            // compute thr2 for third byte
            int thr2 = 0; {
                unsigned int sum = 0; unsigned int need = (unsigned int)(k - sel_count);
                for (int b = 255; b >= 0; --b) {
                    unsigned int sgt = sum;
                    unsigned int eq  = h3[b];
                    if (sgt < need && sgt + eq >= need) { thr2 = b; break; }
                    sum += eq;
                }
            }
            // select those strictly greater on third byte
            for (int i = threadIdx.x; i < N; i += blockDim.x) {
                uint32_t key = float_to_key_desc(col[i]);
                int b0 = (key >> 24) & 0xFF;
                if (b0 != thr0) continue;
            ...
            __syncthreads();
            // final eq gather across first three bytes equalities
            int need_final = k - sel_count;
            if (need_final > 0) {
                eq_count = 0; eq_raw = 0;
                for (int i = threadIdx.x; i < N; i += blockDim.x) {
                    uint32_t key = float_to_key_desc(col[i]);
