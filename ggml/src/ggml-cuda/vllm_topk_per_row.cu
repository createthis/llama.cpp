...
extern "C" void ggml_cuda_topk_per_row(
    ...
    ggml_cuda_vllm_topk::topKPerRow<kNumThreadsPerBlock><<<num_rows, kNumThreadsPerBlock, 0, stream>>>(
        dLogits, dRowStarts, dRowEnds, dOutIndices, stride0, stride1);
}
...
extern "C" void ggml_cuda_topk_per_row_decode(
    ...
    constexpr int kNumThreadsPerBlock = 512;
    cudaStream_t stream = ctx.stream();
    ggml_cuda_vllm_topk::topKPerRowDecode<kNumThreadsPerBlock><<<num_rows, kNumThreadsPerBlock, 0, stream>>>(
        dLogits, dSeqLens, dOutIndices, stride0, stride1, next_n);
}
